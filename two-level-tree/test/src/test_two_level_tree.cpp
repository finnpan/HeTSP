...
static bool is_ordered_neighbor(const TLTS::TwoLevelTree& tree, int a, int b)
{
    return tree.get_next(tree.get_node(a)) == tree.get_node(b) &&
        tree.get_node(a) == tree.get_prev(tree.get_node(b));
}

static bool is_between(const TLTS::TwoLevelTree& tree, int a, int b, int c)
{
    return tree.is_between(tree.get_node(a), tree.get_node(b), tree.get_node(c)); 
}

static std::vector<int> get_tour_via_parents(TLTS::TwoLevelTree& tree,  int start_city)
{
    std::vector<int> ans;
...
void move_2opt(TLTS::TwoLevelTree& tree, int t1, int t2, int t3, int t4)
{
    tree.flip(t1, t2, t4, t3);
}

void undo_2opt_move(TLTS::TwoLevelTree& tree, int t1, int t2, int t3, int t4)
{
...
TEST_CASE("Build tree from an ordered list of cities", "[two level tree]")
{
    ...
    TLTS::TwoLevelTree tree{ n_cities, start_city };
...
TEST_CASE("Prev, Next, and Between", "[two level tree]")
{
    ...
    TLTS::TwoLevelTree tree{ n_cities, origin };
    tree.set_raw_tour(order);
...
TEST_CASE("Split and merge", "[two level tree]")
{
    ...
    tree.split_and_merge(tree.get_node(6), true, TLTS::Direction::forward);
    ...
    REQUIRE(tree.get_raw_tour(10, TLTS::Direction::backward) == std::vector<int>{10, 9, 6, 8, 4, 1, 2, 5,
    ...
    tree.split_and_merge(tree.get_node(4), true, TLTS::Direction::forward);
    ...
    // try backward merge
    tree.split_and_merge(tree.get_node(19), false, TLTS::Direction::backward); //doesn't include 19
    REQUIRE(tree.get_raw_tour(11) == std::vector<int>{11, 13,   5, 2, 1,   4, 8, 6, 9, 10, 7, 12,  
        14, 3, 15, 16, 17, 18, 20,   19, 23, 22, 21});
    ...
    tree.split_and_merge(tree.get_node(10), true, TLTS::Direction::backward);
    REQUIRE(tree.get_raw_tour(11) == std::vector<int>{11, 13,   5, 2, 1, 4, 8, 6, 9, 10,   7, 12,  
        14, 3, 15, 16, 17, 18, 20,   19, 23, 22, 21});
    ...
    tree.split_and_merge(tree.get_node(2), true, TLTS::Direction::forward);
    REQUIRE(tree.get_raw_tour(11) == std::vector<int>{11, 13,   5,    2, 1, 4, 8, 6, 9, 10, 7, 12,  
        14, 3, 15, 16, 17, 18, 20,   19, 23, 22, 21});
    ...
    REQUIRE(tree.get_raw_tour(2, TLTS::Direction::backward) == std::vector<int>{2, 5, 13, 11, 21, 22, 23, 
...
TEST_CASE("Reverse a partial segment with split-and-merge", "[two level tree]")
{
    ...
    REQUIRE(tree.get_parent_node(8)->reverse);
    REQUIRE(tree.get_raw_tour(12, TLTS::Direction::backward) == std::vector<int>{12, 7, 10, 9, 5, 2, 1, 
...
TEST_CASE("Tree deep copy/move and independency", "[two level tree]")
{
    ...
    TLTS::TwoLevelTree tree3 = std::move(tree);
